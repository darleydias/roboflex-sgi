        ...
        <div class="table-responsive">
                ...
                <colgroup>
                    ...
                    <?php if($_settings->userdata('type') == '3' || $_settings->userdata('type') == '1' || $_settings->userdata('type') == '2'): ?>
                        ...
                        inner join receiving_list r on r.form_id = p.id WHERE p.req_grupo = 'Operacional' and r.req_aprov = 0 order by p.`date_created` desc");
                        ...
                        $i = 1;
                        $qry = $conn->query("SELECT r.form_id, p.*,r.data_autorizacao FROM `purchase_order_list` p
                        inner join receiving_list r on r.form_id = p.id WHERE p.req_grupo = 'Financeiro' and r.req_aprov = 0 order by p.`date_created` desc");
                        while ($row = $qry->fetch_assoc()) :?>
                        ...
                        $i = 1;
                        $qry = $conn->query("SELECT r.form_id, p.*,r.data_autorizacao FROM `purchase_order_list` p
                        inner join receiving_list r on r.form_id = p.id WHERE p.req_grupo = 'Negócios' and r.req_aprov = 0 order by p.`date_created` desc");
                        while ($row = $qry->fetch_assoc()) :?>
                            <tr><td class="text-center"><?php echo $i++; ?></td>
                                <td><?php echo $row['po_code'] ?></td>
                                <td class="text-center">
                        ...
                        $i = 1;
                        $qry = $conn->query("SELECT r.form_id, p.*,r.data_autorizacao FROM `purchase_order_list` p
                        inner join receiving_list r on r.form_id = p.id WHERE p.req_grupo = 'Administrativo' and r.req_aprov = 0 order by p.`date_created` desc");
                        while ($row = $qry->fetch_assoc()) :?>
                            <tr><td class="text-center"><?php echo $i++; ?></td>
                                <td><?php echo $row['po_code'] ?></td>
                                <td class="text-center">
                        ...
                        $i = 1;
                        $qry = $conn->query("SELECT r.form_id, p.*,r.data_autorizacao FROM `purchase_order_list` p
                        inner join receiving_list r on r.form_id = p.id WHERE p.req_grupo = 'P&D' and r.req_aprov = 0 order by p.`date_created` desc");
                        while ($row = $qry->fetch_assoc()) :?>
                            <tr><td class="text-center"><?php echo $i++; ?></td>
                                <td><?php echo $row['po_code'] ?></td>
                                <td class="text-center">
                        ...
                        $i = 1;
                        $qry = $conn->query("SELECT r.form_id, p.*,r.data_autorizacao FROM `purchase_order_list` p
                        inner join receiving_list r on r.form_id = p.id WHERE p.req_grupo = 'Assistência Técnica' and r.req_aprov = 0 order by p.`date_created` desc");
                        while ($row = $qry->fetch_assoc()) :?>
                            <tr><td class="text-center"><?php echo $i++; ?></td>
                                <td><?php echo $row['po_code'] ?></td>
                                <td class="text-center">
                        ...
                        $i = 1;
                        $qry = $conn->query("SELECT r.form_id, p.*,r.data_autorizacao FROM `purchase_order_list` p
                        inner join receiving_list r on r.form_id = p.id WHERE p.req_grupo = 'Compras Matérias-primas' and r.req_aprov = 0 order by p.`date_created` desc");
                        while ($row = $qry->fetch_assoc()) :?>
                            <tr><td class="text-center"><?php echo $i++; ?></td>
                                <td><?php echo $row['po_code'] ?></td>
                                <td class="text-center">
...
<br>
    <div class="center text-center">
		<a onclick="window.location='<?php echo base_url ?>admin/?page=autorizacao'" class="btn btn-outline-primary col-md-2"><strong> Voltar </strong></a> <br><br>
