...
<div class="container-fluid">
	<form action="" id="item-form" autocomplete="off">
		...
		<div class="form-group">
			<label for="name" class="control-label">Nome do Serviço *</label>
		...
		<div class="form-group">
			...
			<option <?php echo !isset($supplier_id) ? 'selected' : '' ?> disabled>Selecione a Categoria</option>
			<?php $supplier = $conn->query("SELECT s.name,s.id,s.mat_ou_ser FROM `supplier_list` s
			where status = 1
			AND mat_ou_ser = 0
			order by `name` asc");
...
<div class="form-group">
		...
		<div class="form-group" style="display:none;">
			...
			<option value="1" <?php echo isset($mat_ou_ser) && $mat_ou_ser == 1 ? 'selected' : '' ?>>Material</option>
			<option selected value="0" <?php echo isset($mat_ou_ser) && $mat_ou_ser == 0 ? 'selected' : '' ?>>Serviço</option>
			</select>
		</div>
			...
			$.ajax({
				...
				success:function(resp){
					if(typeof resp =='object' && resp.status == 'success'){
						...
						alert_toast(" Preencha todos os campos obrigatórios *",'error');
						end_loader();
                        console.log(resp)
					}
				}
