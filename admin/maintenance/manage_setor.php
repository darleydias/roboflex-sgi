...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT * from `setor_list` where id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<script>
	$(document).ready(function(){
		$('#setor-form').submit(function(e){
			e.preventDefault();
            var _this = $(this)
			 $('.err-msg').remove();
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Master.php?f=save_setor",
