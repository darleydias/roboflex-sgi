...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(empty($id)){
			...
			if($qry){
				...
				$this->settings->set_flashdata('success','Usuário salvo com sucesso.');
				$resp['status'] = 1;
			}else{
				$resp['status'] = 2;
			}
			...
			if($qry){
				$this->settings->set_flashdata('success','Usuário atualizado com sucesso.');
		...
		if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
			...
			if(!in_array($type,$allowed)){
				$resp['msg'].=" Erro ao importar imagem, formato inválido.";
			}else{
				$new_height = 200; 
				$new_width = 200; 
				...
				$resp['msg'].=" Erro ao importar imagem.";
				}
			}
	...
	public function delete_users(){
		...
		if($qry){
			...
			$this->settings->set_flashdata('success','Usuário excluído com sucesso.');
