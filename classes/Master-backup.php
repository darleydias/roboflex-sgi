...
function delete_requisitante(){
	...
	function save_po(){
		...
		if($save){
			...
			foreach($item_id as $k =>$v){
				if(!empty($data)) $data .=", ";
				$data .= "('{$po_id}','{$v}','{$qty[$k]}','{$price[$k]}','{$unit[$k]}','{$total[$k]}','{$obs_item[$k]}','{$fornecedor_item[$k]}','{$prev_data[$k]}','{$cot1[$k]}','{$cot2[$k]}','{$cot3[$k]}','{$cot4[$k]}','{$cot5[$k]}','{$cot6[$k]}','{$cot7[$k]}','{$cot8[$k]}','{$cot9[$k]}','{$bot1[$k]}','{$ped1[$k]}','{$ped2[$k]}','{$ped3[$k]}','{$ped4[$k]}')";
			}
			if(!empty($data)){
				$this->conn->query("DELETE FROM `po_items` where po_id = '{$po_id}'");
				$save = $this->conn->query("INSERT INTO `po_items` (`po_id`,`item_id`,`quantity`,`price`,`unit`,`total`,`obs_item`,`fornecedor_item`,`prev_data`,`cotacao_1_0`,`cotacao_1_1`,`cotacao_1_2`,`cotacao_2_0`,`cotacao_2_1`,`cotacao_2_2`,`cotacao_3_0`,`cotacao_3_1`,`cotacao_3_2`,`botao_cot1`,`pedido_omie`,`notafiscal_rec`,`recebido_rec`,`conforme_rec`) VALUES {$data}");
				if(!$save){
					$resp['status'] = 'failed';
					if(empty($id)){
					...
					$resp['msg'] = 'PO has failed to save. Error: '.$this->conn->error;
					$resp['sql'] = "INSERT INTO `po_items` (`po_id`,`item_id`,`quantity`,`price`,`unit`,`total`,`obs_item`,`fornecedor_item`,`prev_data`,`cotacao_1_0`,`cotacao_1_1`,`cotacao_1_2`,`cotacao_2_0`,`cotacao_2_1`,`cotacao_2_2`,`cotacao_3_0`,`cotacao_3_1`,`cotacao_3_2`,`botao_cot1`,`pedido_omie`,`notafiscal_rec`,`recebido_rec`,`conforme_rec`) VALUES {$data}";
				}
			}
		}else{
			$resp['status'] = 'failed';
